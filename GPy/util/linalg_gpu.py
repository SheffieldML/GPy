...
try:
    ...
    # log(X)
    log = ElementwiseKernel(
        "double *in, double *out", "out[i] = log(in[i])", "log_element"
    )

    # log(1.0-X)
    logOne = ElementwiseKernel(
        "double *in, double *out", "out[i] = log(1.-in[i])", "logOne_element"
    )
    ...
    mul_bcast = ElementwiseKernel(
        ...
        "out[i] = longer[i]*shorter[i%shorter_size]",
        "mul_bcast",
    )
    ...
    mul_bcast_first = ElementwiseKernel(
        ...
        "out[i] = longer[i]*shorter[i/first_dim]",
        "mul_bcast",
    )

    # sum through the middle dimension (size_2) of a 3D matrix (size_1, size_2, size_3)
    sum_axis = ElementwiseKernel(
        "double *out, double *in, int size_1, int size_2",
        "out[i] += sum_axis_element(in, size_1, size_2, i)",
        "sum_axis",
        preamble="""
    ...
    outer_prod = ElementwiseKernel(
        "double *out, double *v1, double *v2, int v1_size",
        "out[i] = v1[i%v1_size]*v2[i/v1_size]",
        "outer_prod",
    )

    # the outer product between two vectors (out = np.einsum('na,nb->nab',m1,m2) a=dim1, b=dim2 )
    join_prod = ElementwiseKernel(
        "double *out, double *m1, double *m2, int dim1, int dim2",
        "out[i] = m1[(i%dim1)*dim1+(i%(dim1*dim2))/dim1]*m2[(i%dim1)*dim1+i/(dim1*dim2)]",
        "join_prod",
    )
