...
def ln_diff_erfs(x1, x2, return_sign=False):
    ...
    if x1.shape == x2.shape:
        ...
        if x1.size == 1:
            ...
            raise ValueError(
                "This function does not broadcast unless provided with a scalar."
            )
    ...
    with np.errstate(divide="ignore"):
        ...
        I0 = np.logical_or(
            np.logical_and(x1 > 0, x2 < 0), np.logical_and(x2 > 0, x1 < 0)
        )  # I1=(x1*x2)<0
        ...
        I2 = np.logical_and(
            x1 > 0, np.logical_and(np.logical_not(I0), np.logical_not(I1))
        )
        # Case 3: Both arguments are non-positive
        I3 = np.logical_and(
            np.logical_and(np.logical_not(I0), np.logical_not(I1)), np.logical_not(I2)
        )
        _x2 = x2.flatten()
        _x1 = x1.flatten()
        for group, flags in zip((0, 1, 2, 3), (I0, I1, I2, I3)):
            ...
            if np.any(flags):
                ...
                if group == 0:
                    ...
                    v[flags] = (
                        np.log(erfcx(_x2) - erfcx(_x1) * np.exp(_x2 ** 2 - _x1 ** 2))
                        - _x2 ** 2
                    )
                elif group == 3:
                    v[flags] = (
                        np.log(erfcx(-_x1) - erfcx(-_x2) * np.exp(_x1 ** 2 - _x2 ** 2))
                        - _x1 ** 2
                    )
