...
def data_available(dataset_name=None):
    ...
    for file_list, save_list in zip_longest(*zip_urls, fillvalue=[]):
        for f, s in zip_longest(file_list, save_list, fillvalue=None):
            if s is not None:
                f = s  # If there is a save_name given, use that one
...
def download_url(url, store_directory, save_name=None, messages=True, suffix=""):
    ...
    with open(save_name, "wb") as f:
        ...
        while True:
            ...
            if file_size:
                status = r"[{perc: <{ll}}] {dl:7.3f}/{full:.3f}MB".format(
                    dl=file_size_dl / (1048576.0),
                    full=file_size / (1048576.0),
                    ll=line_length,
                    perc="=" * int(line_length * float(file_size_dl) / file_size),
                )
            else:
                status = r"[{perc: <{ll}}] {dl:7.3f}MB".format(
                    dl=file_size_dl / (1048576.0),
                    ll=line_length,
                    perc="."
...
def download_data(dataset_name=None):
    ...
    if "suffices" in dr:
        zip_urls += (dr["suffices"],)
    else:
        zip_urls += ([],)

    for url, files, save_names, suffices in zip_longest(*zip_urls, fillvalue=[]):
        for f, save_name, suffix in zip_longest(
            files, save_names, suffices, fillvalue=None
        ):
...
if gpxpy_available:
    ...
    def epomeo_gpx(data_set="epomeo_gpx", sample_every=4):
        ...
        for file in files:
            ...
            data = [
                [
                    (point.time - datetime.datetime(2013, 8, 21)).total_seconds(),
                    point.latitude,
                    point.longitude,
                    point.elevation,
                ]
...
def google_trends(
    ...
    if not os.path.exists(file_name) or refresh_data:
        ...
        for term in query_terms:
            quoted_terms.append(urlquote(term))
        ...
        data = re.sub(
            ...
            (
                lambda m: '"%s-%02d-%02d"'
                % (m.group(1).strip(), 1 + int(m.group(2)), int(m.group(3)))
            ),
            data,
        )
    ...
    Y = np.asarray(
        [[df.ix[row][query_terms[i]]] for i in range(terms) for row in df.index]
    )
...
def decampos_digits(
    ...
    lbls = np.array([[i] * num_samples for i in which_digits]).reshape(Y.shape[0], 1)
    str_lbls = np.array([[str(i)] * num_samples for i in which_digits])
    return data_details_return(
        {
            "Y": Y,
            "lbls": lbls,
...
def global_average_temperature(
    data_set="global_temperature", num_train=1000, refresh_data=False
):
    ...
    return data_details_return(
        {
            ...
            "Xtest": Xtest,
            "Ytest": Ytest,
            "info": "Montly airline passenger data from Box & Jenkins 1976.",
        },
        data_set,
    )
...
def osu_run1(data_set="osu_run1", sample_every=4):
    ...
    if not data_available(data_set):
        download_data(data_set)
        zp = zipfile.ZipFile(os.path.join(data_path, data_set, "run1TXT.ZIP"), "r")
        for name in zp.namelist():
            zp.extract(name, path)
...
def hapmap3(data_set="hapmap3"):
    ...
    unpacked_files = [
        os.path.join(dir_path, hapmap_file_name + ending) for ending in [".ped", ".map"]
    ]
    unpacked_files_exist = reduce(
        lambda a, b: a and b, map(os.path.exists, unpacked_files)
    )
    ...
    if not reduce(lambda a, b: a and b, map(os.path.exists, preprocessed_data_paths)):
        if not overide_manual_authorize and not prompt_user(
...
def singlecell_rna_seq_islam(dataset="singlecell_islam"):
    ...
    header1 = read_csv(
        filename, sep="\t", header=None, skiprows=5, nrows=1, compression="gzip"
    )
    header2 = read_csv(
        filename, sep="\t", header=None, skiprows=3, nrows=1, compression="gzip"
    )
...
def singlecell_rna_seq_deng(dataset="singlecell_deng"):
    ...
    sample_info = read_csv(
        filename, sep="\t", skiprows=30, compression="gzip", header=0, index_col=0
    ).T
    sample_info.columns = sample_info.columns.to_series().apply(
        lambda row: row[len("!Sample_") :]
    )
    ...
    with tarfile.open(filename, "r") as files:
        ...
        for i, file_info in enumerate(members):
            ...
            message = "{: >7.2%}: Extracting: {}".format(
                float(i + 1) / overall, file_info.name[:20] + "...txt.gz"
            )
            print(message, end=" ")
            if data is None:
...
def toy_linear_1d_classification(seed=default_seed):
    ...
    X = (np.r_[x1, x2])[:, None]
    return {"X": X, "Y": sample_class(2.0 * X), "F": 2.0 * X, "seed": seed}
...
def olivetti_faces(data_set="olivetti_faces"):
    ...
    if not data_available(data_set):
        download_data(data_set)
        zp = zipfile.ZipFile(os.path.join(path, "att_faces.zip"), "r")
        for name in zp.namelist():
            zp.extract(name, path)
    Y = []
    lbls = []
