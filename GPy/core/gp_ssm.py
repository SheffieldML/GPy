...
class GpSSM(GP):
    ...
    def optimize(self, optimizer=None, start=None, **kwargs):
        ...
        while ((change > 0.001) and (count < 50)):
            ...
            self.optimizeParams(expectations=expectations)
            change = np.abs(likelihood - prevLikelihood)
            prevLikelihood = likelihood
            count = count + 1

    def optimizeParams(self, optimizer=None, start=None, expectations=None, **kwargs):
        if self.is_fixed:
        ...
        opt.run(f_fp=self.paramMaximisationStep, args=(self.X, self.Y, expectations))
        self.optimization_runs.append(opt)
        self.optimizer_array = opt.x_opt

    def paramMaximisationStep(self, loghyper, X, Y, E, *args):
