...
class MultioutputLikelihood(MixedNoise):
    ...
    def moments_match_ep(self, data_i, tau_i, v_i, Y_metadata_i):
        moments = self.likelihoods_list[Y_metadata_i["output_index"][0]].moments_match_ep(data_i, tau_i, v_i, Y_metadata_i)
        return tuple(np.asarray(moment).item() for moment in moments)
