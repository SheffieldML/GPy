...
def logisticln(x):  # pragma: no cover
    return np.where(
        x < lim_val,
        np.where(x > -lim_val, -np.log(1 + np.exp(-x)), -x),
        -np.log(1 + epsilon),
    )
...
def clip_exp(x):  # pragma: no cover
    return np.where(
        x < lim_val, np.where(x > -lim_val, np.exp(x), epsilon), 1 / epsilon
    )
...
def differfln(x0, x1):  # pragma: no cover
    ...
    return np.where(
        x0 > x1,
        -x1 * x1 + np.log(erfcx(x1) - np.exp(-(x0 ** 2) + x1 ** 2) * erfcx(x0)),
        -x0 * x0 + np.log(np.exp(-(x1 ** 2) + x0 ** 2) * erfcx(x1) - erfcx(x0)),
    )
