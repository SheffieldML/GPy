...
def chain_3(d3f_dg3, dg_dx, d2f_dg2, d2g_dx2, df_dg, d3g_dx3):
    ...
    """
    if np.all(dg_dx == 1.0) and np.all(d2g_dx2 == 0) and np.all(d3g_dx3 == 0):
        return d3f_dg3
...
def kmm_init(X, m=10):
    ...
    XXT = np.dot(X, X.T)
    D = -2.0 * XXT + np.diag(XXT)[:, np.newaxis] + np.diag(XXT)[np.newaxis, :]
    ...
    def wrapper_func(self, *args, **kwargs):
        ...
        if self.not_block_really and (len(retval.shape) < 3):
            ...
            for param_ind in range(num_params):
                dhess_dtheta[diag_slice, diag_slice, param_ind] = np.squeeze(
                    retval[:, param_ind]
                )
            return dhess_dtheta
        else:
